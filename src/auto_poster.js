  ...
  if (!admin.apps.length) {
    admin.initializeApp({
      credential: admin.credential.cert({
        ...
        clientEmail: process.env.FIREBASE_CLIENT_EMAIL // تصحيح: استخدام المتغير الصحيح
      })
    });
  }
...
class TelegramNotifier {
    constructor() {
        ...
        this.chatId = process.env.TELEGRAM_CHAT_ID; // تصحيح: استخدام المتغير الصحيح
...
class AutoPosterSystem {
  ...
  getCurrentTarget() {
    ...
    // تنسيق الوقت ليتطابق مع مفاتيح الجدول (HH:MM)
    ...
    if (!category) {
        // هذا الخطأ يحدث إذا تم تشغيل السكربت في وقت غير مجدول
  ...
  async run() {
    ...
    if (!contentDoc) {
      // نلقي خطأ محدد إذا لم يتم العثور على محتوى
    ...
    if (result.success) {
      ...
      // نلقي خطأ إذا فشلت جميع محاولات النشر
...
async function main() {
  ...
  try {
    ...
    process.exit(1); // إنهاء العملية مع رمز خطأ للإشارة إلى فشل المهمة في GitHub Actions
  }
}
