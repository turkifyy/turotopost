...
const CONFIG = {
  ...
  TIMEZONE_OFFSET: 4 // توقيت دبي (UTC+4)
};
...
// =============================================================================
// 7. جدولة المحتوى الدقيقة (بتوقيت دبي)
// =============================================================================
...
class AutoPosterSystem {
  ...
  getCurrentTarget() {
    const now = new Date();
    // تحويل الوقت إلى توقيت دبي (UTC+4)
    const dubaiTime = new Date(now.getTime() + (CONFIG.TIMEZONE_OFFSET * 60 * 60 * 1000));
    ...
    if (!dailySchedule[currentTimeKey]) {
      ...
      for (const timeKey of Object.keys(dailySchedule)) {
        ...
        if (difference < minDifference && difference <= 5) {
  ...
  async sendSuccessReport(content, category, period, accountName, timeDifference = 0) {
    ...
    message += `*الفئة:* ${category}\n` +
               ...
               `*الفترة:* ${period}\n` +
               `*الحساب:* ${accountName}\n` +
               `*الوقت:* ${new Date().toLocaleString('ar-SA', { timeZone: 'Asia/Dubai' })} بتوقيت دبي`;
