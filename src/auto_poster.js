 ...
 * مطور بواسطة: Turki
 * التاريخ: 2025
 * الوصف: نظام متقدم للنشر التلقائي باستخدام Firebase و Zapier Premium
 */
...
function validateEnvironmentVariables() {
  ...
  // التحقق من صحة JSON لحسابات Zapier
  try {
...
const CONFIG = {
  ...
  MAX_RETRIES: 3,
  RETRY_DELAY: 2000,
  REQUEST_TIMEOUT: 45000, // 45 ثانية للحسابات Premium
  REPOST_COOLDOWN_DAYS: 15,
  UAE_TIMEZONE_OFFSET: 4, // UTC+4
...
class ZapierAccountManager {
  ...
  async initialize() {
    ...
    // تحميل أو إنشاء إعدادات التناوب
...
class ContentManager {
  ...
  async getContentForCategory(category, limit = CONFIG.MAX_POSTS_PER_CATEGORY) {
    ...
    try {
      // المرحلة 1: محتوى جديد غير منشور
      ...
      // المرحلة 2: محتوى قديم قابل لإعادة النشر
      ...
      // المرحلة 3: أي محتوى متاح (حالة طوارئ)
...
class AutoPosterSystem {
  constructor() {
    ...
    try {
      ...
      // التحقق من وقت التشغيل المناسب (توقيت الإمارات UTC+4)
  ...
  // التحقق من الوقت المناسب حسب توقيت الإمارات
  isValidUaeTime() {
    const now = new Date();
    const uaeHours = now.getUTCHours() + CONFIG.UAE_TIMEZONE_OFFSET;
    const dayOfWeek = now.getUTCDay();
    
    // الأوقات المثلى للنشر (8 AM - 11 PM توقيت الإمارات)
    const isPeakTime = uaeHours >= 8 && uaeHours <= 23;
    
    // الجمعة له أوقات مختلفة (2 PM - 11 PM)
    ...
    try {
      ...
      for (const contentDoc of contentDocs) {
        ...
        if (result.success) {
          ...
          // تحديث حالة المحتوى في Firebase
          ...
          // تأخير بين المنشورات لتجنب الإيقاف
