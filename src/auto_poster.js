...
function validateEnvironmentVariables() {
  const required = ['FIREBASE_PROJECT_ID', 'FIREBASE_PRIVATE_KEY', 'FIREBASE_CLIENT_EMAIL', 'ZAPIER_WEBHOOKS'];
...
const { format, subDays, getUTCDay, getUTCHours, getUTCMinutes } = require('date-fns');
...
class ZapierAccountManager {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!doc.exists) {
        throw new Error(`لم يتم العثور على جدول تناوب الحسابات لليوم (${today}). تأكد من تشغيل مهمة 'setup_daily_rotation'.`);
      }
  ...
  getAccountForPeriod(period) {
    if (!this.dailyAssignment || !this.dailyAssignment.assignments[period]) {
      throw new Error(`لا يوجد حساب مخصص للفترة '${period}'`);
    }
...
class AutoPosterSystem {
  ...
  getCurrentTarget() {
    ...
    const hours = String(getUTCHours(now)).padStart(2, '0');
    const minutes = String(getUTCMinutes(now)).padStart(2, '0');
    const timeKey = `${hours}:${minutes}`;

    const category = POSTING_SCHEDULE[day]?.[timeKey];
    if (!category) {
        throw new Error(`لا يوجد فئة مجدولة لهذا الوقت: ${timeKey} UTC`);
    }
  ...
  async run() {
    try {
      ...
      if (!contentDoc) {
        throw new Error(`لم يتم العثور على محتوى للفئة: ${category}`);
      }
      ...
      if (result.success) {
        ...
        throw new Error(`فشل النشر النهائي بعد ${CONFIG.MAX_RETRIES} محاولات. الخطأ: ${result.error}`);
      }
    } catch (error) {
      console.error('💥 خطأ فادح في عملية التشغيل:', error.message);
      await telegramNotifier.send(`💥 *خطأ في نظام النشر*\n\n*الرسالة:*\n\`${error.message}\``);
      throw error; // Re-throw to make the GitHub Action fail
    }
  }
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('🎉 بدء تشغيل نظام النشر التلقائي v5.0');
