...
/**
 * نظام النشر التلقائي على فيسبوك v5.2 - توقيت دبي
 * مطور بواسطة: Turki
 * تاريخ التحديث: 2025
...
const CONFIG = {
  ...
  DUBAI_UTC_OFFSET: 4 // توقيت دبي (UTC+4)
};
...
// =============================================================================
// 7. جدولة المحتوى الدقيقة (بتوقيت دبي UTC+4)
// =============================================================================
const POSTING_SCHEDULE = {
  ...
  3: { // الأربعاء
    ...
    '18:30': 'مسلسل', '19:00': 'مباراة', '19:45': 'قناة',
...
class ContentManager {
  ...
  async fetchNewContent(category) {
    ...
    return this.db.collection('links')
      ...
      .where('importStatus', 'in', ['ready', 'pending', 'completed']) // قبول حالات متعددة
...
class AutoPosterSystem {
  ...
  getCurrentTarget() {
    const now = new Date();
    // تحويل الوقت من UTC إلى توقيت دبي (UTC+4)
    const dubaiTime = new Date(now.getTime() + (CONFIG.DUBAI_UTC_OFFSET * 60 * 60 * 1000));
    
    const day = dubaiTime.getUTCDay();
    ...
    // الحصول على الجدول الزمني لليوم الحالي
    const dailySchedule = POSTING_SCHEDULE[day] || {};
    
    // البحث عن الوقت الحالي أو الأقرب في الجدول (بهامش ±5 دقائق)
    let targetTimeKey = currentTimeKey;
    let timeDifference = 0;
    
    if (!dailySchedule[currentTimeKey]) {
      console.log(`⚠️ الوقت الحالي (${currentTimeKey}) غير موجود في الجدول، البحث عن أقرب وقت...`);
      ...
      for (const timeKey of Object.keys(dailySchedule)) {
        const [timeHours, timeMinutes] = timeKey.split(':').map(Number);
        const timeTotalMinutes = timeHours * 60 + timeMinutes;
        const difference = Math.abs(timeTotalMinutes - currentMinutes);
        
        if (difference < minDifference && difference <= 5) { // هامش خطأ: 5 دقائق
      ...
      if (closestTime) {
        targetTimeKey = closestTime;
        timeDifference = minDifference;
        console.log(`✅ تم العثور على أقرب وقت: ${targetTimeKey} (بفارق ${minDifference} دقيقة)`);
      } else {
        throw new Error(`لا يوجد وقت مجدول قريب من الوقت الحالي (${currentTimeKey}) ضمن هامش الخطأ المسموح (5 دقائق).`);
      }
    }

    const category = dailySchedule[targetTimeKey];
    if (!category) {
      throw new Error(`لا يوجد فئة مجدولة للوقت: ${targetTimeKey} بتوقيت دبي. يرجى التحقق من جدول cron.`);
    }

    let period;
    const hourNum = parseInt(targetTimeKey.split(':')[0], 10);
    ...
    if (timeDifference > 0) {
      console.log(`🎯 [5/11] الهدف المعدل: اليوم=${day}, الوقت الأصلي=${currentTimeKey}, الوقت المستهدف=${targetTimeKey} بتوقيت دبي, الفترة=${period}, الفئة=${category} (بفارق ${timeDifference} دقيقة)`);
    } else {
      console.log(`🎯 [5/11] الهدف الحالي: اليوم=${day}, الوقت=${targetTimeKey} بتوقيت دبي, الفترة=${period}, الفئة=${category}`);
    }
    
    return { category, period, timeDifference };
  }

  async run() {
    const { category, period, timeDifference } = this.getCurrentTarget();
    ...
    if (result.success) {
      ...
      await this.sendSuccessReport(contentData, category, period, zapierAccount.name, timeDifference);
    } else {
  ...
  async sendSuccessReport(content, category, period, accountName, timeDifference = 0) {
    console.log('📊 [10/11] إرسال تقرير النجاح...');
    
    let message = `✅ *تم النشر بنجاح*\n\n`;
    
    if (timeDifference > 0) {
      message += `⚠️ *ملاحظة:* تم النشر بفارق ${timeDifference} دقيقة عن الوقت المخطط\n\n`;
    }
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('🎉 بدء تشغيل نظام النشر التلقائي v5.2 - توقيت دبي');
